...
def show_kymograph(df):
    ...
    binwidth = 1
    # plt.hist2d(x, y,
    #            bins=[range(min(x), max(x) + binwidth + 1, binwidth), range(min(y), max(y) + binwidth + 1, binwidth)],
    #            cmap='gray')
    plt.hist2d(y, x,
...
file_name = '/0_code/number_control/changing_AF/master/states/lineage0.csv'
