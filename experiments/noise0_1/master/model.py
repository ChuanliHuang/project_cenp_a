...
def dilute(old_state, noise0_1, bias):
    new_state = np.copy(old_state)
    h3_positions = np.argwhere(old_state == 0).flatten()
    for j in h3_positions:
        if random.random() < noise0_1:
            new_state[j] = 1
    one_positions = np.argwhere(new_state == 1).flatten()
...
if __name__ == '__main__':
    ...
    for lineage in range(config.lineages_to_simulate):
        ...
        for generation in range(config.generations_to_simulate):
            ...
            state = dilute(state, config.noise0_1, config.dilutionBias)
            # observe
            states.append(state)
