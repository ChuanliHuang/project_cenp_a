...
def dilute(old_nuc_comp, lr, distribution_bias):
    nuc_comp = np.copy(old_nuc_comp)
    cenp_a_positions = np.argwhere(nuc_comp == 1).flatten()
    cooperating_positions = np.random.randint(0, len(nuc_comp)-1, len(cenp_a_positions))
...
if __name__ == '__main__':
    ...
    for lineage in range(config.simulation.lineages_to_simulate):
        ...
        for generation in range(config.simulation.lineage.generations_to_simulate):
            ...
            state = dilute(state, config.simulation.lineage.generation.loss_rate, config.simulation.lineage.generation.dist_bias)
            # observe
            states.append(state)
