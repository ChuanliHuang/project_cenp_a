...
def replenish(old_nuc_comp, mu, sigma, af):
    nuc_comp = np.copy(old_nuc_comp)
    cenp_a_positions = np.argwhere(nuc_comp == 1).flatten()
    chosen_position = random.choice(cenp_a_positions)
    print(chosen_position)
    random_shift = generate_probability_function(mu, sigma, af)
    distances_to_deposit = random_shift(1)[0]
    new_position = chosen_position + distances_to_deposit
    print(new_position)
    nuc_comp[new_position % len(nuc_comp)] = 1
    return nuc_comp
...
def update():
    global state
    for i in range(500):
        state = replenish(state, mu, sigma, amplitude_factor)
    state = dilute(state, dist_bias)


pycxsimulator.GUI(parameterSetters=[loading_efficiency, loading_range, initial_density,
