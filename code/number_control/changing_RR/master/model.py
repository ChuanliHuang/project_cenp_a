...
def replenish(old_nuc_comp, target_density, mu, sigma, af):
    nuc_comp = np.copy(old_nuc_comp)
    counter = 0
    while nuc_comp.sum() / nuc_comp.shape[-1] <= target_density:
...
if __name__ == '__main__':
    ...
    for lineage in range(config.simulation.lineages_to_simulate):
        ...
        for generation in range(config.simulation.lineage.generations_to_simulate):
            # update
            state, rr_num = replenish(state,
                                      config.simulation.lineage.target_density, config.simulation.lineage.generation.mu,
        ...
        # output rr_nums
        output_data(rr_nums, 'rr_num', lineage)
