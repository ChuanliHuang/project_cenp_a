...
def replenish(old_state, rr, s, a, b):
    ...
    for i in range(rr):
        ...
        if len(zero_positions) == len(old_state):
            break
...
def dilute(old_state, bias, threshold, noise):
    density = old_state.sum() / len(old_state)
    new_state = np.copy(old_state)
    cenp_a_positions = np.argwhere(new_state == 1).flatten()
    h3_positions = np.argwhere(new_state == 0).flatten()
...
if __name__ == '__main__':
    ...
    for lineage in range(config.lineages_to_simulate):
        ...
        for generation in range(config.generations_to_simulate):
            ...
            state = dilute(state, config.dilutionBias, config.stabilizationThreshold, config.noise_0_1)
            # observe
            states.append(state)
