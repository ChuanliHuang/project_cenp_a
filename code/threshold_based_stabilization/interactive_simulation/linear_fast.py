...
def noise_0_1(val=beta):
    global beta
    beta = float(val)
    return val


def stabilization_threshold(val=gamma):
    global gamma
    gamma = float(val)
    return val
...
def dilute(old_nuc_comp, distribution_bias, stabilization_threshold, noise_0_1):
    density = old_nuc_comp.sum() / len(old_nuc_comp)
    nuc_comp = np.copy(old_nuc_comp)
    cenp_a_positions = np.argwhere(nuc_comp == 1).flatten()
    h3_positions = np.argwhere(nuc_comp == 0).flatten()
    if density > 2 * stabilization_threshold:
        for i in cenp_a_positions:
            if random.random() < distribution_bias:
                nuc_comp[i] = 0
        for j in h3_positions:
            if random.random() < noise_0_1:
                nuc_comp[j] = 1
    else:
        for i in cenp_a_positions:
            nuc_comp[i] = 0
...
def update():
    ...
    state = dilute(state, dist_bias, gamma, beta)


pycxsimulator.GUI(parameterSetters=[loading_efficiency, rounds_of_replenishment, loading_range, initial_density,
                                    number_of_nucleosomes, distribution_bias, noise_0_1, stabilization_threshold]).start(func=[initialize, observe, update])
