...
def observe():
    ...
    arr1 = np.tile(state, (int(len(state) * 0.1), 1))
    arr2 = np.zeros((int(len(state) * 0.05), int(len(state))))
    plotting_array = np.concatenate((arr2, arr1, arr2), axis=0)
    plt.cla()
    plt.imshow(plotting_array, cmap='gray')
